...
namespace ChamCong_v03 {
	public partial class frm_52_PhanQuyen : Form {
		...
		private DataTable LayDSTaikhoan() {
			...
			#endregion
			DataTable kq;
			kq = SqlDataAccessHelper.ExecuteQueryString(query, new string[] { "@UserID" }, new object[] { XL2.currUserID });
			return kq;
		}
		...
		private List<cChucNang> TaoChucNang() {
			...
			lstChucNang.Add(new cChucNang() { ID = 10001, MoTa = "Xem Công, chỉnh sửa giờ và xuất báo biểu" });
			...
			lstChucNang.Add(new cChucNang() { ID = 20002, MoTa = "Chấm công tay cho Nhân viên" });
			lstChucNang.Add(new cChucNang() { ID = 20003, MoTa = "Chấm công tay cho Quản lý" });
		...
		private void frm_PhanQuyen_Load(object sender, EventArgs e) {
			...
			if (XL2.currUserID != int.MaxValue) m_TablePhongban = LayDSPhongban(XL2.currUserID);
			...
			if (XL2.currUserID != int.MaxValue) lstTaikhoan.DataSource = LayDSTaikhoan();
		...
		private void lstTaikhoan_SelectedValueChanged(object sender, EventArgs e) {
			...
			DataTable table1 = (XL2.currUserID != int.MaxValue) ? LayDSCacPhongban(Selected_UserID, XL2.currUserID) : LayDSCacPhongban(Selected_UserID);
