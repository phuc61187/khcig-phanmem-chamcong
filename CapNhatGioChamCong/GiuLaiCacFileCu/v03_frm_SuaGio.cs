	...
	public partial class v03_frm_SuaGio : Form {
		...
		private void btnSuaGio_Click(object sender, EventArgs e) {
			...
			for (int i = 0; i < arrRows.Length; i++) {
				...
				TimeStr = check.Time;
				bool kq = false;// DAL.SuaGioChoNV(iUserEnrollNumber, check.TimeStr, ngay, (check.MachineNo % 2 == 1) ? true : false, check.Source, check.MachineNo, check.PhucHoi.IDGioGoc, ThamSo.currUserID, lydo, ghichu);
		...
		private void btnDaoNguoc_Click(object sender, EventArgs e) {
			...
			for (int i = 0; i < arrRows.Length; i++) {
				...
				bool kq = false;// DAL.SuaGioChoNV(iUserEnrollNumber, check.TimeStr, check.TimeStr, (check.MachineNo % 2 == 1) ? false : true, check.Source, check.MachineNo, check.PhucHoi.IDGioGoc, ThamSo.currUserID, lydo, ghichu);
