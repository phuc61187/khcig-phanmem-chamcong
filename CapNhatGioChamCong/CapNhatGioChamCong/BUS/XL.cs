...
namespace CapNhatGioChamCong.BUS {
	public static class XL {
		...
		public static void XemCong(List<cUserInfo> pDSNV, DateTime pNgayBD, DateTime pNgayKT) {
			...
			try {
				...
				foreach (cUserInfo nhanvien in pDSNV) {
					...
					DuaVaoNgayCong(nhanvien.DSVaoRa, pNgayBD.Date, pNgayKT.Date, nhanvien.DSNgayCong, nhanvien.MacDinhTinhPC150);
		...
		private static void GhiCongLamViecNgayNghi(List<cNgayCong> list, DataRow[] subTableLamViecNgayNghi) {
			if (subTableLamViecNgayNghi.Length == 0) return;
			foreach (var row in subTableLamViecNgayNghi) {
				DateTime ngay = (DateTime)row["Ngay"];
				Single HeSoPhucap = Convert.ToSingle(row["HeSoPC"].ToString());
				cNgayCong ngayCong = list.Find(o => o.NgayCong.Date == ngay);
				//ngayCong.PhuCapDem = (((float)(ngayCong.TongGioLamDem.TotalHours / 8f)) * 0.3f);
				ngayCong.PhuCapDem = Math.Round(((ngayCong.TongGioLamDem.TotalHours / 8d) * 0.3d), 2);
				ngayCong.PhuCapThem = (HeSoPhucap * ngayCong.TongCong) + ((ngayCong.TongGioLamDem.TotalHours / 8d) * 0.3d);
				ngayCong.TongPhuCap = ngayCong.PhuCapDem + ngayCong.PhuCapThem; // reset lại từ đầu = 0
				ngayCong.TongPhuCap = Math.Round(ngayCong.TongPhuCap, 2);
			}
		}
		...
		private static void GhiCongChamTay(List<cNgayCong> DSNgayCong, DataRow[] subTableNgayChamCongTay) {
			foreach (var row in subTableNgayChamCongTay) {
				DateTime ngay = (DateTime)row["Ngay"];
				Single Cong = Convert.ToSingle(row["Cong"].ToString());
				Single Phucap = Convert.ToSingle(row["PhuCap"].ToString());
				cNgayCong ngayCong = DSNgayCong.Find(o => o.NgayCong.Date == (DateTime)row["Ngay"]);
				ngayCong.TongCong += Cong;
				ngayCong.TongCong = Math.Round(ngayCong.TongCong, 2);
		...
		private static void GhiCongVang(List<cNgayCong> DSNgayCong, DataRow[] arrRowNgayVang) {
			foreach (DataRow row in arrRowNgayVang) {
				cLoaiVang loaiVang = new cLoaiVang() {
					...
					Cong = Convert.ToSingle(row["Workingday"].ToString()),
				...
				switch (loaiVang.KyHieu) {
					...
					case "L":
						...
						ngay.TongCong = Math.Round(ngay.TongCong, 2);
						break;
				}
		...
		private static void TinhCongChoViec(cUserInfo nv, DateTime ngayDauThang) {
			...
			Double tong = Math.Ceiling(nv.TongCongThang) + nv.TongCongBH + nv.TongCongRo;
		...
		public static void TinhCongTheoCa(cChkInOut pCheckINOUT, cShift ca) {
			...
			kq.Cong = ((kq.TGLamTinhCong.TotalHours / ca.WorkingTimeTS.TotalHours) * ca.Workingday); // chia cho workingtime vì hcb làm 3 tiếng nhưng vẫn tính 1 công
			...
			if (kq.TGLamDem < Properties.Settings.Default.TGLamDemToiThieu) {
				kq.TGLamDem = ThamSo._0gio;
			}
			if (kq.TGLamDem == ThamSo._0gio) pCheckINOUT.QuaDem = true;
			else pCheckINOUT.QuaDem = false;

			kq.PhuCapDem = ((kq.TGLamDem.TotalHours / 8d) * 0.3d);
		}
		...
		public static void TinhCongKoTheoCa(cChkInOut pCheckINOUT, cShift ca) {
			...
			kq.Cong = (kq.TGLamTinhCong.TotalHours / 8d); // chia 8 tiếng
			if (kq.RaaLam > tmpBDLamDem) {
				DateTime tmpKTLamDem = tmpDate.AddDays(1d).Add(ThamSo._05h45);
		...
		public static void TaoCaTuDo(cShift pCa, DateTime pCheckInTime, TimeSpan pWorkingTime, TimeSpan pchophepvaotreTS, TimeSpan pchopheprasomTS, TimeSpan pbatdaulamthemTS, Single pWorkingDay) {
		...
		public static void DuaVaoNgayCong(List<cChkInOut> pDSVaoRa, DateTime ngayBD, DateTime ngayKT, List<cNgayCong> pDSNgayCong, bool macdinhtinhpc50) {
			DateTime ngaydem;
			if (pDSVaoRa.Count == 0) {
				ngaydem = ngayBD.Date;
			...
			while (ngaydem <= ngayKT.Date) {
				...
				cNgayCong ngayCOcheck = new cNgayCong() {
					HasCheck = true, NgayCong = ngayCoMat, MacDinhTinhPC50 = macdinhtinhpc50,// mặc định khởi tạo ngày có tính PC 50%, nếu tồn tại ít nhất 1 ca nào ko tính pc 50% thì set cả ngày ko tính pc 50%
					...
					PhuCapDem = ((CIO.TGLamDem.TotalHours > 0d)
					? ((CIO.TGLamDem.TotalHours / 8d) * 0.3d)
					: 0d)
				...
				if (CIO.TinhPC150 != macdinhtinhpc50) ngayCOcheck.MacDinhTinhPC50 = CIO.TinhPC150;
				if (ngayCOcheck.MacDinhTinhPC50) {
					ngayCOcheck.PhuCapThem = (((CIO.TGLamTinhCong.TotalHours > 8d)
														   ? (((CIO.TGLamTinhCong.TotalHours - 8d) / 8d) * 0.5d)
														   : 0d));
				}
				else ngayCOcheck.PhuCapThem = 0d;

				ngayCOcheck.TongPhuCap = ngayCOcheck.PhuCapDem + ngayCOcheck.PhuCapThem;
				ngayCOcheck.TongPhuCap = Math.Round(ngayCOcheck.TongPhuCap, 2);
				...
				while (vtriBD < pDSVaoRa.Count && ThamSo.GetDate(pDSVaoRa[vtriBD].TimeStrDaiDien) == ngayCOcheck.NgayCong) {
					...
					ngayCOcheck.TongCong = Math.Round(ngayCOcheck.TongCong, 2);
					ngayCOcheck.PhuCapDem = ((ngayCOcheck.TongGioLamDem.TotalHours > 0d)
												   ? ((ngayCOcheck.TongGioLamDem.TotalHours / 8d) * 0.3d)
												   : 0d);
					#endregion
					if (CIO_1.QuaDem == true) ngayCOcheck.QuaDem = true; // set qua đêm nếu có
					if (CIO_1.TinhPC150 != macdinhtinhpc50) ngayCOcheck.MacDinhTinhPC50 = CIO_1.TinhPC150;
					if (ngayCOcheck.MacDinhTinhPC50) {
						ngayCOcheck.PhuCapThem = (((CIO_1.TGLamTinhCong.TotalHours > 8d)
															   ? (((CIO_1.TGLamTinhCong.TotalHours - 8d) / 8d) * 0.5d)
															   : 0d));
					}
					else ngayCOcheck.PhuCapThem = 0d;
		...
		public static bool XoaGioChoNV(cUserInfo tmpNhanvien, cChk pCheck, string pLydo, string pGhichu) {
			int kq = 0;
			DateTime timeafter30min = pCheck.TimeStr.Add(ThamSo._30phut);
			kq = SqlDataAccessHelper.ExecNoneQueryString(ThamSo.DelStrXoaGioVaoRa(), new string[] { "@UserEnrollNumber", "@TimeStrOld", "@MachineNoOld", "@TimeAfter30Min", "@Source", "@UserID", "@Explain", "@Note", "@CommandType" },
														 new object[] { tmpNhanvien.UserEnrollNumber, pCheck.TimeStr, pCheck.MachineNo, timeafter30min, pCheck.Source, ThamSo.currUserID, pLydo, pGhichu, -1 });
			if (kq == 0) return false;
		...
		public static void BUS_XacNhan(cUserInfo pNV, cChkInOut pOldCIO, cChkInOut NewCIO, bool pIsOT, int pSoPhutLamThem, TimeSpan pSoPhutLamThemTS, bool pTinhPC150) {
			...
			Single tmpWKDay = NewCIO.ThuocCa.Workingday;
...
*/
			...
			SqlDataAccessHelper.ExecNoneQueryString(ThamSo.InsStrXacNhanCa(),
													...
													new object[] { pNV.UserEnrollNumber, iNewShiftID, pNewShift.ShiftCode, pNewShift.OnnDutyTS.ToString(@"hh\:mm"), pNewShift.OffDutyTS.ToString(@"hh\:mm")
                                                                 ...
                                                                 , dOldTimeStrVao, dOldTimeStrRa, pSoPhutLamThem, "Xác nhận ca và làm thêm"});
		...
		public static void BUS_TachCaVaXacNhan(cUserInfo pNV, cChkInOut pOldCIO, cChkInOut[] pNewCIO, bool pIsOT, int pSoPhutLamThem, TimeSpan pSoPhutLamThemTS, bool pTinhPC150) {
			...
			ThemGioChoNV(pNV, CIO1.Raa.TimeStr, 22, string.Empty, "Tách giờ vào ra ca 3&1");
			ThemGioChoNV(pNV, CIO2.Vao.TimeStr, 21, string.Empty, "Tách giờ vào ra ca 3&1");
		...
		public static void TableCIO_To_DSCheck(DataRow[] arrRow, cUserInfo nhanvien, DateTime pNgayBD, DateTime pNgayKT) {
			...
			foreach (DataRow row in arrRow) {
				...
				if (row["IDXacNhanCaVaLamThem"] == DBNull.Value)
					...
					Single tmpWKDay = Convert.ToSingle(row["Workingday"].ToString());
					...
					nhanvien.Check_GioDaXN(tmpTimeStr, tmpSource, tmpMachineNo,
										   ...
										   tmpOn, tmpOff, tempLateGrace, tempEarlyGrace, tempAfterOT, tempTinhPC150
		...
		public static bool KhaiBaoNgayVangChoNV(object[,] DSMaCC_MaNV_Checked, List<DateTime> DSNgayCheck, DataRowView row) {
			...
			Single cong = Convert.ToSingle(row["WorkingDay"].ToString());
			Single workingtime = Convert.ToSingle(row["WorkingTime"].ToString());
		...
		public static int ChamCongTay(int iMaCC, DateTime ngay, Single fCong, Single fPhucap) {
		...
		public static bool KhaiBaoLVNgayNghiChoNV(object[,] arrDSNVCheck, DateTime NgayKhaiBao, Single hesoPC) {
			...
			for (int i = 0; i < dsCIO.Count; i++) {
				...
				if (ca != null) {
					if (ca.Workingday == 2f && ca.OnnDutyTS > ThamSo._20h00) {
						cChk raaca3 = new cChkOut() { GioLienQuan = null, MachineNo = 22, Source = "PC", TimeStr = CIO.Vao.TimeStr.Date.Add(ca.OnnDutyTS).Add(ThamSo._8gio) };
						cChk vaoca1 = new cChkIn() { GioLienQuan = null, MachineNo = 21, Source = "PC", TimeStr = CIO.Vao.TimeStr.Date.Add(ca.OnnDutyTS).Add(ThamSo._8gio1giay) };
						CIO.Raa = raaca3;
						i = i - 1;
