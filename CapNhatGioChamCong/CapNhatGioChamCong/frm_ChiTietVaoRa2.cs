...
namespace CapNhatGioChamCong {
    ...
    public partial class frm_ChiTietVaoRa2 : Form {
        ...
        private void dgrdTongHop_RowEnter(object sender, DataGridViewCellEventArgs e) {
            //string test = "RowEnter=" + e.RowIndex;
            DataTable tmpTable = (dgrdTongHop.DataSource as DataTable);
            if (tmpTable == null || tmpTable.Rows.Count == 0) return;
            if (e.RowIndex == -1) return;
            DataRowView row = dgrdTongHop.Rows[e.RowIndex].DataBoundItem as DataRowView;
            dgrdTongHop.Tag = row;
            if (row == null) return;
            //test += "; Rowview:" + dataRowView.Row.ToString();
            ...
            DateTime tmpVao = (row["TimeStrVao"] == DBNull.Value) ? DateTime.MinValue : (DateTime)row["TimeStrVao"];
            DateTime tmpRa = (row["TimeStrRa"] == DBNull.Value) ? DateTime.MinValue : (DateTime)row["TimeStrRa"];
            ...
            try {
				...
				//LoadTabThemGio1(ngay, tmpVao, tmpRa, tmpNV.DSCa);
		...
		private void LoadTabThemGio1(DateTime ngay, DateTime tmpVao, DateTime tmpRa, List<cShift> list) {
        ...
        private void btnThemGio_Click(object sender, EventArgs e) {
	        string temp = "dgrdTongHop.Rows.GetRowCount " +dgrdTongHop.Rows.GetRowCount(DataGridViewElementStates.Selected).ToString();
	        temp += "\nindex of first row=" + dgrdTongHop.Rows.GetFirstRow(DataGridViewElementStates.Selected);
			AutoClosingMessageBox.Show(temp,"",500);return;
            ...
            if (checkVao_Them.Checked) {
                dtpVao_Them.Update();
                if (XL.ThemGioChoNV(tmpNV, dtpVao_Them.Value, 21, cbLyDo_Them.Text, tbGhiChu_Them.Text) == false)
                    MessageBox.Show("Không thêm được giờ vào cho nhân viên. Vui lòng thử lại.", "Lỗi");
            }
            if (checkRa_Them.Checked) {
                dtpRa_Them.Update();
                if (XL.ThemGioChoNV(tmpNV, dtpRa_Them.Value, 22, cbLyDo_Them.Text, tbGhiChu_Them.Text) == false)
