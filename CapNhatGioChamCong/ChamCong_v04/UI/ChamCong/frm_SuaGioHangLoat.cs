		...
		private void btnXem_Click(object sender, EventArgs e) {
			...
			try //general try catch
			{
				...
				try {
					...
					fDataTableChkInOut = DAO.LayTableCIO_A(Arr_MaCC, startTime, endTime);
			...
			try
			{
				...
				if (frm311Thongtin.fOK) {
					...
					foreach (DataRow row in tmpArrRow) {
						...
						flag = DAO.SuaGioChoNV(tmpUserEnrollNumber, tmpGioCu, tmpGioMoi, tmpSourceOld, "PC", tmpMachineNo, (tmpKieuGioMoi) ? 21 : 22, -1, tmpLyDo, tmpGhiChu);
						if (flag == false) {
